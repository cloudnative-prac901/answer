...
    });
    ...
    this.albSg.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(80), 'Allow HTTP from Internet');  // 本番リスナー用ポート
    this.albSg.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(443), 'Allow HTTPS from Internet');  // HTTPS移行用ポート
    this.albSg.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(9001), 'Allow test listener (9001) from Internet');  // テストリスナー用ポート
    this.albSg.addEgressRule(this.ecsSg, ec2.Port.tcp(80), 'ALB-to-ECS');

    // ALB2: InternetからHTTPを許可  ★追加
    this.alb2Sg.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(80), 'Allow HTTP from Internet');  // 本番リスナー用ポート
    this.alb2Sg.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(9001), 'Allow test listener (9001) from Internet');  // テストリスナー用ポート
